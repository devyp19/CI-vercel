...
const VotesPage = () => {
  ...
  React.useEffect(() => {
    ...
    //   callApi6(); //get particular applicant's data
    //   callApi5(); //get google response of a particular employe
    //   callApi7(); //status update of a particular applicant
    //   AllAppcallApi();//employee tab-> tech_tag
    //   AllAppcallApi4(); //status update where applicantId->10 count
    //   AllAppcallApi9(); //status update where applicantId->10 count, status=true
    //   checkVotes();
    // }
    if (tabIndex == 0) {
      MyVotescallApi4();
  ...
  //     setResult((AllAppapiData9?.[0]?.count / AllAppapiData?.[0]?.count) * 100);
  //     //upsert
  //   } else setResult("Under Process...");
  // }
